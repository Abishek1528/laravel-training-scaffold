    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="flex space-x-2">
                @can('update', $task)
                    <a href="{{ route('tasks.edit', [$task->project, $task]) }}" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                        Edit
                    </a>
                @endcan
                @can('delete', $task)
                    <form action="{{ route('tasks.destroy', [$task->project, $task]) }}" method="POST" onsubmit="return confirm('Are you sure?')">
                        @csrf
                        @method('DELETE')
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6 text-gray-900">
                    ...
                    @if($task->attachment_path)
                        <div class="mt-6">
                            <h5 class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Attachment</h5>
                            <a href="{{ asset('storage/' . $task->attachment_path) }}" download class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-500 active:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                Download {{ basename($task->attachment_path) }}
                            </a>
                        </div>
                    @endif
                </div>
